...
import { whatsappGonder, mesajMetni } from './WhatsAppMenu';
...
export default function TopluCubuk({
  ...
  temizle: () => void;
  waBaslat: (sablon: Sablon | null) => void;
}) {
  ...
  return (
    <div className="toplu-cubuk" role="region" aria-label="Toplu işlem">
      ...
      {sablonlar.length > 0 && (
        <select className="toplu-secim wa-secim" defaultValue="" aria-label="WhatsApp ile sırayla yaz"
                onChange={(e) => {
                  const v = e.target.value;
                  e.target.value = '';
                  if (v === 'bos') waBaslat(null);
                  else { const s = sablonlar.find((x) => String(x.id) === v); if (s) waBaslat(s); }
                }}>
          <option value="" disabled>💬 WhatsApp…</option>
          {sablonlar.map((s) => <option key={s.id} value={s.id}>{s.title}</option>)}
          <option value="bos">Boş mesaj</option>
        </select>
      )}
...
/**
 * Seçilenlere sırayla WhatsApp: tarayıcı aynı anda birden çok sekme
 * açmaya izin vermediği için her kişi için bir dokunuş. Sabit hat gibi
 * WhatsApp'ı olmayan numaralar kendiliğinden atlanır.
 */
export function WaKuyruk({
  adaylar, sablon, kullanici, marka, kaydet, bitir,
}: {
  ...
  const uygun = adaylar.filter((a) => telefonCoz(a.phone_raw).whatsapp);
    ...
    return (
      <div className="wa-kuyruk">
        <div className="wa-kuyruk-ust">
          ...
          {atlanan > 0 && <span className="cell-sub"> · {atlanan} kişinin WhatsApp&apos;ı yok (sabit hat / numara yok)</span>}
        </div>
  ...
  return (
    <div className="wa-kuyruk" role="dialog" aria-label="WhatsApp kuyruğu">
      <div className="wa-kuyruk-ust">
        <span className="kuyruk-sayac">WhatsApp · {sira + 1} / {uygun.length}{sablon ? ` · ${sablon.title}` : ''}</span>
        <strong>{simdiki.name}</strong>
        <div className="wa-onizleme">{mesajMetni(sablon, simdiki, kullanici.ad, marka.ad) || '(boş mesaj)'}</div>
      </div>
      <div className="wa-kuyruk-eylem">
        <button type="button" className="btn btn-success"
                onClick={() => {
                  if (whatsappGonder(simdiki, sablon, kullanici.ad, marka.ad, kaydet)) setGonderilen((v) => v + 1);
                  ilerle();
                }}>
          WhatsApp&apos;ı aç
        </button>
