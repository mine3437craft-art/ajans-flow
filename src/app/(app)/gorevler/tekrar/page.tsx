...
import { createTemplate, updateTemplate, toggleTemplate, deleteTemplate } from '../tekrar-actions';
...
type Sablon = {
  ...
  customer_id: number | null; assigned_to: number | null;
...
export default async function TekrarPage() {
  ...
  const sablonlar = (await sql`
    SELECT s.id, s.title, s.description, s.weekdays, s.priority, s.is_active,
           s.customer_id, s.assigned_to, c.name AS customer_name, u.display_name AS assignee_name
